...
const Empdashboard = () => {
    ...
    const getMonthDates = () => {
        ...
        for (let d = 1; d <= lastDay.getDate(); d++) { dates.push(new Date(year, month, d)); } return dates;
    }; const monthDates = getMonthDates(); const todayDate = new Date().getDate(); return (<div className="p-2 md:p-2 bg-slate-50 dark:bg-slate-900 min-h-screen text-slate-800 dark:text-slate-100 space-y-6">
  ...
  dark:hover:shadow-none">
                ...
                <div className="grid grid-cols-7 gap-1 mt-3 text-md">
                    ...
                    {monthDates.map(date => {
                        ...
                        return (
                            <div key={date.toISOString()} className={`p-2 h-10 flex items-center justify-center rounded ${isToday ? "bg-red-800 text-white" : "bg-red-50 dark:bg-slate-700"}`}>
                                {date.getDate()}
                            </div>
                        );
                    })}
